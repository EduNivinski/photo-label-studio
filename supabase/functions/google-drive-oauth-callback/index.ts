...
serve(async (req) => {
  ...
  try {
    ...
    if (!code) {
      return new Response(JSON.stringify({ ok:false, reason:"BAD_CODE" }), {
        status: 400, headers: { ...H, "Content-Type":"application/json" }
      });
    }
    if (!state) {
      return new Response(JSON.stringify({ ok:false, reason:"BAD_STATE" }), {
        status: 400, headers: { ...H, "Content-Type":"application/json" }
      });
    }
...
<body style="font-family: system-ui; margin: 24px">
  Conexão realizada. Você pode fechar esta janela.
    ...
    const isState = /state/i.test(msg);
    const code = isState ? 400 : 500;
